<div>
                ...
                <div class="preview">
                    <div class="overflow-x-auto">
                        <table class="table">
                            ...
                            <tbody>
                                @forelse ($telefonos as $telf )
                                    <tr class=" dark:bg-dark-1 {{ $loop->index % 2> 0 ? 'bg-gray-200' : '' }}">

                                        <td class="dark:border-dark-5">
                                            <h6 class="mb-1 font-medium">{{ $telf->serie }}</h6>

                                        </td>

                                        <td class="dark:border-dark-5">
                                            <h6 class="mb-1 font-medium">{{ $telf->af }}</h6>
                                            {{-- <small class="font-normal">{{ $unidad->unidades->count() }} unidades en este edificio</small> --}}
                                        </td>
                                        <td class="dark:border-dark-5">
                                            <h6 class="mb-1 font-medium">{{ $telf->modelo->nombre }}</h6>
                                            {{-- <small class="font-normal">{{ $unidad->unidades->count() }} unidades en este edificio</small> --}}
                                        </td>

                                        <td class="dark:border-dark-5">
                                            <h6 class="mb-1 font-medium">{{ $telf->usuario }}</h6>
                                        ...
                                        <td class="dark:border-dark-5 text-center">
                                            <div class="d-flex justify-content-center">
                                                {{-- @if ($unidad->unidades->count() < 1) --}}
                                                    ...
                                                    onclick="destroy('telefonos','Destroy', {{ $telf->id }})"
                                                    type="button">
                                                        <i class=" fas fa-trash f-2x"></i>
                                                    </button>
                                                {{-- @endif --}}
                                                <button class="btn btn-warning text-white border-0 ml-3"
                                                    wire:click.prevent="Edit({{ $telf->id }})"
                                                    type="button">
    ...
    @else
        ...
        @include('livewire.telefonos.form')
