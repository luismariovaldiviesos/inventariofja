<div>
                            ...
                            <tbody>
                                @forelse ($monitores as $monitor )
                                    <tr class=" dark:bg-dark-1 {{ $loop->index % 2> 0 ? 'bg-gray-200' : '' }}">
                                        ...
                                        <td class="dark:border-dark-5">
                                            <h6 class="mb-1 font-medium">{{ $monitor->marca }}</h6>
                                            {{-- <small class="font-normal">{{ $monitor->unidades->count() }} unidades en este edificio</small> --}}
                                        </td>

                                        <td class="dark:border-dark-5">
                                            <h6 class="mb-1 font-medium">{{ $edificio->user }}</h6>
                                        ...
                                        <td class="dark:border-dark-5 text-center">
                                            <div class="d-flex justify-content-center">
                                                {{-- @if ($edificio->unidades->count() < 1) --}}
                                                    ...
                                                    onclick="destroy('monitores','Destroy', {{ $monitor->id }})"
                                                    type="button">
                                                        <i class=" fas fa-trash f-2x"></i>
                                                    </button>
                                                {{-- @endif --}}
                                                <button class="btn btn-warning text-white border-0 ml-3"
                                                    wire:click.prevent="Edit({{ $monitor->id }})"
                                                    type="button">
    ...
    @else
        ...
        @include('livewire.monitores.form')
