                            ...
                            <tbody>
                                @forelse ($edificios as $edificio )
                                    <tr class=" dark:bg-dark-1 {{ $loop->index % 2> 0 ? 'bg-gray-200' : '' }}">
                                        ...
                                        <td class="dark:border-dark-5">
                                            <h6 class="mb-1 font-medium">{{ $edificio->id }}</h6>

                                        </td>

                                        <td class="dark:border-dark-5">
                                            <h6 class="mb-1 font-medium">{{ $edificio->nombre }}</h6>
                                            <small class="font-normal">{{ $edificio->unidades->count() }} unidades en este edificio</small>
                                        </td>

                                        <td class="dark:border-dark-5">
                                            <h6 class="mb-1 font-medium">{{ $edificio->canton }}</h6>

                                        </td>

                                        <td class="dark:border-dark-5 text-center">
                                            <div class="d-flex justify-content-center">
                                                @if ($edificio->unidades->count() < 1)
                                                    <button class="btn btn-danger text-white border-0"
                                                    onclick="destroy('edificios','Destroy', {{ $edificio->id }})"
                                                    type="button">
                                                        <i class=" fas fa-trash f-2x"></i>
                                                    </button>
                                                @endif
                                                <button class="btn btn-warning text-white border-0 ml-3"
                                                    wire:click.prevent="Edit({{ $edificio->id }})"
                                                    type="button">
                                ...
                                @empty
                                    <tr class="bg-gray-200 dark:bg-dark-1">
                                        <td colspan="2">
                                            <h6 class="text-center">    NO HAY EDIFICIOS REGISTRADAS </h6>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
