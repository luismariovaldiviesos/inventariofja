                            ...
                            <tbody>
                                @forelse ($modelos as $modelo )
                                    <tr class=" dark:bg-dark-1 {{ $loop->index % 2> 0 ? 'bg-gray-200' : '' }}">
                                        ...
                                        <td class="dark:border-dark-5">
                                            <h6 class="mb-1 font-medium">{{ $modelo->id }}</h6>
                                        ...
                                        <td class="dark:border-dark-5">
                                            ...
                                            {{-- @foreach ($modelo->tipos  as  $tipo)
                                            <h6 class="mb-1 font-medium">{{ $tipo->nombre }}</h6>
                                            @endforeach --}}
                                            <h6 class="mb-1 font-medium">{{ $modelo->tipo->nombre }}</h6>
                                        </td>
                                        <td class="dark:border-dark-5">

                                            <h6 class="mb-1 font-medium">{{ $modelo->marca->nombre }}</h6>
                                        </td>
                                        ...
                                        <td class="dark:border-dark-5">
                                            <div class="d-flex justify-content-center">
                                                {{-- @if ($marca->tipos->count() < 1) --}}
                                                    ...
                                                    onclick="destroy('modelos','Destroy', {{ $modelo->id }})"
                                                    type="button">
                                                        <i class=" fas fa-trash f-2x"></i>
                                                    </button>
                                                {{-- @endif --}}
                                                <button class="btn btn-warning text-white border-0 ml-3"
                                                    wire:click.prevent="Edit({{ $modelo->id }})"
                                                    type="button">
                                ...
                                @empty
                                    <tr class="bg-gray-200 dark:bg-dark-1">
                                        <td colspan="2">
                                            <h6 class="text-center">    NO HAY modelos registrados </h6>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
