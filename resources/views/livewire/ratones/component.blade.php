                ...
                <div class="preview">
                    <div class="overflow-x-auto">
                        <table class="table">
                            ...
                            <tbody>
                                @forelse ($ratones as $raton )
                                    <tr class=" dark:bg-dark-1 {{ $loop->index % 2> 0 ? 'bg-gray-200' : '' }}">

                                        <td class="dark:border-dark-5">
                                            <h6 class="mb-1 font-medium">{{ $raton->serie }}</h6>

                                        </td>

                                        <td class="dark:border-dark-5">
                                            <h6 class="mb-1 font-medium">{{ $raton->af }}</h6>
                                            {{-- <small class="font-normal">{{ $unidad->unidades->count() }} unidades en este edificio</small> --}}
                                        </td>

                                        <td class="dark:border-dark-5">
                                            <h6 class="mb-1 font-medium">{{ $raton->usuario }}</h6>
                                        ...
                                        <td class="dark:border-dark-5 text-center">
                                            <div class="d-flex justify-content-center">
                                                {{-- @if ($unidad->unidades->count() < 1) --}}
                                                    ...
                                                    onclick="destroy('ratones','Destroy', {{ $raton->id }})"
                                                    type="button">
                                                        <i class=" fas fa-trash f-2x"></i>
                                                    </button>
                                                {{-- @endif --}}
                                                <button class="btn btn-warning text-white border-0 ml-3"
                                                    wire:click.prevent="Edit({{ $raton->id }})"
                                                    type="button">
                                ...
                                @empty
                                    <tr class="bg-gray-200 dark:bg-dark-1">
                                        <td colspan="2">
                                            <h6 class="text-center">    NO HAY MOUSES REGISTRADOS </h6>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
